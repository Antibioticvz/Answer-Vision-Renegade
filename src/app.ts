...
if (process.env.MODEL === "openai" && process.env.OPENAI_API_KEY) {
  ...
  if (!systemPrompt)
    throw new Error("OPENAI_SYSTEM_PROMPT environment variable is not set")

  service = {
    getAnswerFromText: async text => {
      const answer = await openaiService.getAnswerFromText(
        model,
        systemPrompt,
        text
      )
  ...
  if (!systemPrompt)
    throw new Error("LLAMA_SYSTEM_PROMPT environment variable is not set")

  service = {
    getAnswerFromText: async text =>
      llamaService.getAnswerFromText(systemPrompt, text),
  }
}
